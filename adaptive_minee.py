...
def importance_resample(data, batch_size, replace=False, prob=None):
    # Resample the given data sample.
    index = np.random.choice(
        range(data.shape[0]), size=batch_size, replace=replace, p=prob)
    batch = data[index]
    batch_prob = prob[index]
    batch_prob = batch_prob/sum(batch_prob)
    return batch, batch_prob
