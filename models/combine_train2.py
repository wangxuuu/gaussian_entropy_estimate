...
class CrossTrain():
    ...
    def __init__(self, X, Y, batch_size=32, ref_batch_factor=1, lr=1e-3, hidden_size=100):
        ...
        self.XY_net = CrossTrain.Net(input_size=X.shape[1]+Y.shape[1], hidden_size=100)
...
#        dX = _div(self.X_net, self.X, batch_X) # why self.X instead of batch?
...
#        self.total_optimizer.step()
    ...
    def stepA(self):
        ...
        dXY = _div(self.XY_net, batch_XY, batch_XY_ref)
        lossXY = -dXY
        lossXY.backward()
        self.XY_optimizer.step()

        dX = _div(self.X_net, batch_X, batch_X_ref)
        lossX = -dX
        lossX.backward()
        self.X_optimizer.step()

        dY = _div(self.Y_net, batch_Y, batch_Y_ref)
        lossY = -dY
        lossY.backward()
        self.Y_optimizer.step()
    ...
    def stepB(self):
        ...
        batch_X_ref = _resample(self.X, batch_size=self.batch_size)
        batch_Y_ref = _resample(self.Y, batch_size=self.batch_size)
        batch_XY_ref = torch.cat((batch_X_ref, batch_Y_ref), dim=1)
    ...
    def forwardA(self, X=None, Y=None):
        """
        Outputs:
        ...
        #return [dXY - dX - dY, dXY, dX, dY]
        return dXY - dX - dY

    def forwardB(self, X=None, Y=None):
        """
        Outputs:
            dXY: divergence of sample joint distribution of (X,Y)
                to the uniform reference
        ...
        if X is None or Y is None:
            ...
            XY = torch.cat((X, Y), dim=1)
        XY_ref = torch.cat((_resample(X,batch_size=X.shape[0]), _resample(Y,batch_size=Y.shape[0])), dim=1)
